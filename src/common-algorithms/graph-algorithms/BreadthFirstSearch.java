...
class BreadthFirstSearch {
    ...
    /**
     * Performs a breadth first search on a graph that's represented in the form
    ...
    public static List<List<Integer>> adjacencyMatrixBFS(int[][] graph) {
        ...
        for(int searchRoot = 0; searchRoot < graph.length; searchRoot++) {
            ...
            // Add the root of the traversal to the queue, and mark the root as visited
            verticesToVisit.add(searchRoot);
            ...
            while(!verticesToVisit.isEmpty()) {
                ...
                int currVertex = verticesToVisit.remove();
                currComponentVisitOrder.add(currVertex);
                // Add the non-visited neighbors of the currVertex into the queue 
    ...
    /**
     * Performs a breadth first search on a graph that's represented in the form of an
     ...
     * were visited for every connnected component of the input graph.
     */
    public static List<List<Integer>> adjacencyListBFS(List<Set<Integer>> graph) {
        // Order in which the vertices were visited for every connected component
        List<List<Integer>> visitOrder = new ArrayList<>();
        // The vertices enqueued to be visited
        Queue<Integer> verticesToVisit = new LinkedList<>();
    ...
    public static void main(String[] args) {
        ...
        List<List<Integer>> matrixOrder = adjacencyMatrixBFS(adjMatrix);
        printVisitOrder(matrixOrder);

        System.out.println("List Order");
        List<List<Integer>> listOrder = adjacencyListBFS(graph.getAdjacencyList());
